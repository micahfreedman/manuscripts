...
setwd('~/Documents/GitHub/Sites/manuscripts/Cardenolide_Sequestration/data/')
...
pdf('../figures/fig5b.pdf',height = 7, width = 3)
...
marianas.leaf.cardenolides <- marianas.leaf.cardenolides[marianas.leaf.cardenolides$species=='Asclepias_curassavica',]
...
pdf('../figures/fig5c.pdf',height = 7, width = 3)
...
nmds1 <- metaMDS(comm = nmds.guam, distance = 'bray', k = 3, trymax = 500) #generally converges in <200 iterations

stressplot(nmds1)

plot.nmds.guam <- cbind(marianas.wing.cardenolides.full[,1:3], scores(nmds1))

#pdf('../figures/figS1.pdf',height = 6, width = 6)
